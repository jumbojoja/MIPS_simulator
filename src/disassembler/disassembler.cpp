...
/*int main()
{
    ...
    string fileObj;
    string fileAsm;
    return_by_dis disassembleRet; // The disassembleLine function return struct
(a bool and string) int lineNum = 0;              // Keeps track of the line
number int addressLineNum;           // This holds the address line numbers
calculated when an address is encountered at a given position map<int,
std::string> addMap; // Map holds pairs of addresses and lineNumbers
    ...
    while (getline(objStream, line))
    {
        ...
        {
            cout << "Cannot disassemble " << line << " at line " << lineNum <<
endl;
        }
        else
        {
            ...
            if (disassembleRet.branch_address)
            {
                ...
                {
                    addressReturn temp =
addressStrForm(disassembleRet.addressOffset, lineNum); string label =
temp.hexStr; addMap[addressLineNum] = label; // 添加标签
                }
            }
        }
        lineNum++;
    ...
    if (disassembleRet.errorFound == false)
    {
        ...
        {
            disassembleRet = disassembleLine(line, lineNum);
            for (const auto &pair : addMap) // 转汇编输出时看是不是需要跳转的
            {
...
return_by_dis r_type_decoder(string binaryLine) {
  ...
  // Uses the functReturn to figure out how to assess instruction (shamt or no
  // shamt)
...
return_by_dis i_type_decoder(string binaryLine, string opcode, int lineNum) {
  ...
  // If an error is found here the funct is not valid and returns funct return
  // with error
...
int get_imm(string immediateStr) {
  ...
  if (immediateStr[0] == '1') // 负数
  {
    ...
    int immNum = stoi(immediateStr, nullptr, 2);
  }
  return immNum;
}
