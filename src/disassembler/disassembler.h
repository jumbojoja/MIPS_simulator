...
enum instructFormat { i_type, r_type };
...
// Converts the binary into instructions while checking that the instruction is
// valid
...
// Converts 2 hex digits into an I-instruction opcode, also checks for invalid
// opcode
opcodeReturn hexOpcode(char hex1, char hex2);

// Finds the funct from the binary value, finds if shamt or a 3rd register is
// assessed, spits out an error if not a valid funct
functReturn findfunct(string funct);

// Finds the register from the binary value, will spit out an error if not a
// valid register
