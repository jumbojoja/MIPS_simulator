...
string To_string(int x, int scale, int num_of_str, int character = 0) // 十进制转二进制(数，进制，位数，是否符号扩展(默认不扩展))
{
    int flag = 0, num = 0;
    string s = "";
    string tmp;
...
void Assembly_to_Machine(string *mem)
{
    ...
    while (mem[i] != "")
    {
        ...
        if (part[0] == "addi" || part[0] == "addiu" || part[0] == "andi" || part[0] == "ori" || part[0] == "xori" || part[0] == "lui" || part[0] == "lw" || part[0] == "sw" || part[0] == "beq" || part[0] == "bne" || part[0] == "slti" || part[0] == "sltiu") // I型指令
        {
        ...
        if (part[0] == "addiu" || part[0] == "andi" || part[0] == "ori" || part[0] == "xori" || part[0] == "lui" || part[0] == "lw" || part[0] == "sw" || part[0] == "beq" || part[0] == "bne" || part[0] == "slti" || part[0] == "sltiu") // J型指令
        {
            ...
            if (part[0] == "lw" || part[0] == "sw" || part[0] == "beq" || part[0] == "bne")
                Machine_code = opcode + To_string(rd, 2, 5) + To_string(rs, 2, 5) + To_string(rt, 2, 16, 1);
            else
                Machine_code = opcode + To_string(rs, 2, 5) + To_string(rd, 2, 5) + To_string(rt, 2, 16);
        }
        else if (part[0] == "addi")
        {
            Machine_code = opcode + To_string(rs, 2, 5) + To_string(rd, 2, 5) + To_string(rt, 2, 16, 1); // addi按符号位扩展
        }
        else if (part[0] == "j" || part[0] == "jal") // J型
        {
            // auto it = find(lable.begin(), lable.end(), type_of_rd); //??????????/   引入 algorithm！！！！！！！！！！！！！！！！
            for (int i = 0; i < lable.size(); i++)
            {
        ...
        {
            Machine_code = opcode + "00000" + To_string(rs, 2, 5) + To_string(rd, 2, 5) + To_string(rt, 2, 5) + funct;
        }
        ...
        {
            Machine_code = opcode + To_string(rs, 2, 5) + To_string(rt, 2, 5) + To_string(rd, 2, 5) + "00000" + funct;
        }

        // cout << "机器码有" << Machine_code.size() << "位:" << Machine_code << endl;
...
int main()
{
    int i = 0, j = 0;
    string l;
    string mem[1000]; // string不能初始化为NULL
    getline(cin, l);
    do
    {
        mem[i] = l;
    ...
    // cout << To_string(12, 2, 16, 1);

    // cout << To_string(255, 16, 8);

    // cout << sizeof(mem) / sizeof(mem[0]);
    // cout << "来啊" << mem[550];
